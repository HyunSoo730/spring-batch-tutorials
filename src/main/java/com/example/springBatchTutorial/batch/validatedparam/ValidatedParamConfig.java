package com.example.springBatchTutorial.batch.validatedparam;
...
public class ValidatedParamConfig {
    ...
    public Job validatedParamJob(Step validatedParamStep) {
        Job validatedParamJob = jobBuilderFactory.get("validatedParamJob")  //이름을 정해주고, get에 써놓은 Job 이름으로 배치를 실행하게 될꺼야.
                .incrementer(new RunIdIncrementer())  //Job을 실행할 때 id를 부여하는데, Sequence를 순차적으로 부여할 수 있도록 RunIdIncrementer를 해주자.
                .validator(new FileParamValidator())
                .start(validatedParamStep)   //Job안에는 Step이 존재해야해.
                .build();
        return validatedParamJob;
    }
    ...
    public Step validatedParamStep(Tasklet validatedParamTasklet) {
        ...
        TaskletStep helloWorldStep = stepBuilderFactory.get("helloWorldStep")  //역시 똑같이 get을 통해 이름 명시
                .tasklet(validatedParamTasklet)     //일단 간단하게 taskLet으로 해보자.
                .build();
